...
describe("定刻の開店 → 解答 → 閉店", () => {
  ...
  it("開店時刻ちょうどで開く", () => {
    ...
    expect(effects.some((e) => e.k === "norenOpen")).toBe(true);
  ...
  it("解答すると結果画面に移り、記録が solved になる", () => {
    const { state, effects } = run(s, [
      ...
      { type: "ANSWER", answer: "She has been waiting", accuracy: 0.89 },
    ]);
    expect(state.screen).toBe("result");
    expect(state.session).toBeNull();
    expect(state.outcome!.accuracy).toBe(0.89);
    expect(state.outcome!.timedOut).toBe(false);
    expect(todayRecord(state)!.solved).toBe(true);
    expect(todayRecord(state)!.accuracy).toBe(0.89);
    expect(effects.some((e) => e.k === "norenClose")).toBe(true);
  });

  it("結果を閉じると閉店中に戻る（「もう一度」は無い）", () => {
    const after = run(s, [
      { type: "TICK", nowMs: OPEN_AT },
      { type: "ANSWER", answer: "x", accuracy: 0.5 },
  ...
  it("解いた後は開店枠の中でも再び開かない", () => {
    const after = run(s, [
      { type: "TICK", nowMs: OPEN_AT },
      { type: "ANSWER", answer: "x", accuracy: 1 },
      { type: "RESULT_DISMISSED" },
